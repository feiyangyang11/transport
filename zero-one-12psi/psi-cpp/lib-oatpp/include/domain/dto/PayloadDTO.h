...
public:
	...
	void setAvatar(std::string val) { avatar = val; }
	std::string getFrameId() const { return frameId; }
	void setFrameId(std::string val) { frameId = val; }
	std::string getFrameName() const { return frameName; }
	void setFrameName(std::string val) { frameName = val; }
	...
	void propToJwt(T* obj) const
	{
		...
		obj->add_claim("id", id);
		// TIP：新增字段在后面补充即可
		obj->add_claim("avatar", avatar);
		obj->add_claim("frameId", frameId);
		obj->add_claim("frameName", frameName);
	}
	...
	void propToPayload(jwt::jwt_object* obj)
	{
		...
		// TIP：新增字段在后面补充即可
		if (_payload.contains("avatar"))
			setAvatar(_payload["avatar"].get<std::string>());
		if (_payload.contains("frameId"))
			setFrameId(_payload["frameId"].get<std::string>());
		if (_payload.contains("frameName"))
			setFrameName(_payload["frameName"].get<std::string>());
	}
};
